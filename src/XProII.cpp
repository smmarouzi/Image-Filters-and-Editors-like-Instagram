#include "..\include\XProII.h"
#include <math.h>

#define pi 3.14159265


XProII::XProII(std::string inputPath, std::string outputPath)
{
...
XProII::~XProII()
{
	std::string currOutputPath = outputFilePath + "/\\" + "XproII" + ".jpg";
	cv::imwrite(currOutputPath, result);
	std::cout << "Successfully written output image at " << currOutputPath << std::endl;
}

int XProII::run(float xproiiScale)
{
	float sepiaScale = 0;//Between 0 and 1, 0:original image, 1:full sepia image
	float brightnessScale = 1 + xproiiScale*0.4;//brightnessScale is between 0 and 2, 0: black, 0-1: darker, 1: original image, 1-2: brighter, 2: white
	float contrastScale = 1.25;//contrastScale a scale between 0 and 3. 0: black, 1: no effect,2: twice contrast
	float saturationScale = 1.3;//saturationScale, 0:gray image, no saturation, 1: original image, 1-3: x times saturation
	int degree = -5;//degree (any range)
...
void XProII::display()
{
	cv::imshow("XproII:visuals", result);
...
int main()
{
	XProII xproii("../GLoFilters/data/tree.jpg", "../GLoFilters/result");
	xproii.run(0.2);
	xproii.display();
