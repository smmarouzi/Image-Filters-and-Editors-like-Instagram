#include "..\include\Ludwig.h"



Ludwig::Ludwig(std::string inputPath, std::string outputPath)
{
...
Ludwig::~Ludwig()
{
	std::string currOutputPath = outputFilePath + "/\\" + "ludwig" + ".jpg";
	cv::imwrite(currOutputPath, result);
	std::cout << "Successfully written output image at " << currOutputPath << std::endl;
}

int Ludwig::run(float ludwigScale)
{
	float sepiaScale = 0.25;//Between 0 and 1, 0:original image, 1:full sepia image
	float brightnessScale = 1+ ludwigScale*0.4;//brightnessScale is between 0 and 2, 0: black, 0-1: darker, 1: original image, 1-2: brighter, 2: white
	float contrastScale = 1.05;//contrastScale a scale between 0 and 3. 0: black, 1: no effect,2: twice contrast
	float saturationScale = 2.0;//saturationScale, 0:gray image, no saturation, 1: original image, 1-3: x times saturation
...
void Ludwig::display()
{
	cv::imshow("Ludwig:visuals", result);
...
int main()
{
	Ludwig ludwig("../GLoFilters/data/tree.jpg", "../GLoFilters/result");
	ludwig.run(0.2);
	ludwig.display();
